...
function jetOauth(config = {}) {
    ...
    return async function (req, res, next) {
        ...
        if (
            req.method == "GET" &&
            /^\/jetoauth\/(login|callback)\/(.+)$/.test(req.path)
        ) {
            const provider = req.path.replace(
                /^\/jetoauth\/(login|callback)\//,
                ""
            );
            switch (true) {
                ...
                case /^\/jetoauth\/callback\/(.+)$/.test(req.path):
                    if (plugins[provider]) {
                        ...
                        if (result) {
                            res.redirect(
                                config?.redirect?.success || "/jetoauth/success"
                            );
                            return next(false);
                        } else {
                            res.redirect(
                                config?.redirect?.fail || "/jetoauth/fail"
                            );
                            return next(false);
                        }
                    }
        ...
        if (
            req.method == "GET" &&
            /^\/jetoauth\/logout$/.test(req.path) &&
            req.jetOauth.isLogin
        ) {
            req.jetOauth.logout();
